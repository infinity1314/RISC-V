...
//    input       [31:0]        Data_Mem_Address      ，
    input        [31:0]     Data_Mem_WriteData ,        // 6位数码管要显示的数值
...
//wire   [31:0]     Data_Mem_WriteData_1 ;
//wire define
wire   [3:0]              data0    ;        // 个位数
wire   [3:0]              data1    ;        // 十位数
wire   [3:0]              data2    ;        // 百位数
...
//assign Data_Mem_Address=local;
//*****************************************************
//**                    main code
//*****************************************************

//提取显示数值所对应的十进制数的各个位
assign  data0 = Data_Mem_WriteData % 4'd10;               // 个位数
assign  data1 = Data_Mem_WriteData / 4'd10 % 4'd10   ;    // 十位数
assign  data2 = Data_Mem_WriteData / 7'd100 % 4'd10  ;    // 百位数
assign  data3 = Data_Mem_WriteData / 10'd1000 % 4'd10 ;   // 千位数
assign  data4 = Data_Mem_WriteData / 14'd10000 % 4'd10;   // 万位数
assign  data5 = Data_Mem_WriteData / 17'd100000;          // 十万位数
...
//        seg_led <= 8'hc0;
//        end
//    else if(Data_Mem_Address==local)begin
//        seg_sel<=Data_Mem_WriteData;
//         seg_led<=Data_Mem_WriteData;
//         end
...
always @ (posedge dri_clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if(1) begin
            case (cnt_sel)
                3'd0 :begin
                 //   seg_sel  <= 6'b111110;  //显示数码管最低位
                    seg_sel  <= 6'b000001;  //显示数码管最低位
                    num_disp <= num[3:0] ;  //显示的数据
                    dot_disp <= ~point[0];  //显示的小数点
                end
                3'd1 :begin
                    //seg_sel  <= 6'b111101;  //显示数码管第1位
                     seg_sel  <= 6'b000010;  //显示数码管第1位
                    num_disp <= num[7:4] ;
                    dot_disp <= ~point[1];
                end
                3'd2 :begin
                   //seg_sel  <= 6'b111011;  //显示数码管第2位
                    seg_sel  <= 6'b000100;  //显示数码管第2位
                    num_disp <= num[11:8];
                    dot_disp <= ~point[2];
                end
                3'd3 :begin
                   // seg_sel  <= 6'b110111;  //显示数码管第3位
                     seg_sel  <= 6'b001000;  //显示数码管第3位
                    num_disp <= num[15:12];
                    dot_disp <= ~point[3];
                end
                3'd4 :begin
                   // seg_sel  <= 6'b101111;  //显示数码管第4位
                   seg_sel  <= 6'b010000;  //显示数码管第4位
                    num_disp <= num[19:16];
                    dot_disp <= ~point[4];
                end
                3'd5 :begin
                    seg_sel  <= 6'b100000;  //显示数码管最高位
                    num_disp <= num[23:20];
                    dot_disp <= ~point[5];
                end
                default :begin
            ...
            seg_sel  <= 6'b000000;      //使能信号为0时，所有数码管均不显示
            num_disp <= 4'b0;
            dot_disp <= 1'b1;
        end
    end
